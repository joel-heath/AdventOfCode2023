...
{
    public static readonly string ProjectName = Assembly.GetCallingAssembly().GetName().Name!; // Should be AdventOfCode[YEAR] (since its calling assembly name wont be null)
    public static readonly string Year = ProjectName[^4..]; // Should be AdventOfCode[YEAR] (since its calling assembly name wont be null)
    ...
    static async Task<string> FetchInput(int day)
    {
        ...
        if (responseBody[^2] == '\r') responseBody = responseBody[..^2];
        else if (responseBody[^1] == '\n') responseBody = responseBody[..^1];

        return responseBody.ReplaceLineEndings();
    }
    ...
    {
        string startupPath = FindSolutionPath(); //  ASSUMING WE ARE IN AdventOfCode2023\AdventOfCode2023\bin\Debug\net8.0\ it would be Directory.GetParent(AppContext.BaseDirectory)!.Parent!.Parent!.Parent!.Parent!.FullName;
        IDay day;
        int part;
