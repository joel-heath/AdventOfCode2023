    ...
    public string SolvePart2(string input)
        => $"{new Grid<char>[] { new(input.Split(Environment.NewLine).Select(l => l.ToCharArray()).ToArray()) }
            .Select(grid => (grid, Enumerable.Range(0, 4).Select(
                c => new IEnumerable<long>[] { Utils.Range(0, grid.Width) }.Select(r => c == 3 ? r.Reverse() : r).First()
            ...
            .Select(input => 
        Utils.Range(1_000_000_000 - 1, -1_000_000_000).AggregateWhile<(Grid<char> grid, int lb, List<string> history, long i), long>
            ...
            , acc => acc.lb == -1)).Select(results => (results.history[results.lb..^1], results.i)).Select(results => results.Item1[(int)results.i % results.Item1.Count])
