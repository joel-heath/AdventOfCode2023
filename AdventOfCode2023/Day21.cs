    ...
    public string SolvePart2(string input)
        => $"{((input[0] != '.') // test inputs
            ? new string[] { string.Concat(input.TakeWhile(x => x != '.')) }
                .Select(num => (num, d:new char[][][] { input[num.Length..].Split(Environment.NewLine).Select(l => l.ToCharArray()).ToArray() }
            .Select(map => (map, start:map.SelectMany((r, i) => r.Select((c, j) => (c, new Point(i, j)))).First(t => t.c == 'S').Item2)).First()))
                .Sum(d => Solve(d.d.map, d.d.start, long.Parse(d.num)))
            : new (char[][] map, Point start)[] { new char[][][] { input.Split(Environment.NewLine).Select(l => l.ToCharArray()).ToArray() }
            .Select(map => (map, map.SelectMany((r, i) => r.Select((c, j) => (c, new Point(i, j)))).First(t => t.c == 'S').Item2)).First() }.Sum(z => 
